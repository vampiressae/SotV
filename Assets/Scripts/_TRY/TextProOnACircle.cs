...
{
    private enum SnapTo { Center, Top, Bottom, Left, Right }

    /// <summary>
    /// Flips the letters upside down
    /// </summary>
    [SerializeField]
    [Tooltip("Flips the letters upside down")]
    private bool m_flip;

    /// <summary>
    /// Snap the text to a side
    /// </summary>
    [SerializeField]
    [Tooltip("Snap the text to a side")]
    private SnapTo m_snapTo;
    ...
    private SnapTo m_oldSnapTo = SnapTo.Center;

    /// <summary>
    /// Previous value of <see cref="m_flip"/>
    /// </summary>
    private bool m_oldFlip = false;
    ...
    protected override bool ParametersHaveChanged()
    {
        ...
        bool retVal = 
            m_radius != m_oldRadius || m_arcDegrees != m_oldArcDegrees || m_angularOffset != m_oldAngularOffset ||
            m_oldMaxDegreesPerLetter != m_maxDegreesPerLetter || m_oldFlip != m_flip || m_oldSnapTo != m_snapTo;

        m_oldSnapTo = m_snapTo;
        m_oldFlip = m_flip;
    ...
    protected override Matrix4x4 ComputeTransformationMatrix(Vector3 charMidBaselinePos, float zeroToOnePos, TMP_TextInfo textInfo, int charIdx)
    {
        ...
        float x0 = Mathf.Cos(angle) * flipper;
        ...
        switch (m_snapTo)
        {
            case SnapTo.Top: newMideBaselinePos.y += m_radius; break;
            case SnapTo.Bottom: newMideBaselinePos.y += m_radius; break;
            case SnapTo.Left: newMideBaselinePos.x -= m_radius; break;
            case SnapTo.Right: newMideBaselinePos.x += m_radius; break;
        }

        var atan = -Mathf.Atan2(y0 * flipper, x0 * flipper);
        ...
        return Matrix4x4.TRS(new Vector3(newMideBaselinePos.x, newMideBaselinePos.y, 0), Quaternion.AngleAxis(atan * Mathf.Rad2Deg - 90, Vector3.forward), Vector3.one);
    }
}